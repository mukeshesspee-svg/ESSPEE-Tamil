...
import { Copy, Trash2, Type, Download, FileText, File as FileIcon } from "lucide-react";
...
export function TanglishEditor() {
  ...
  // Simple transliteration using Google Input Tools API
    ...
    <div className="flex flex-col gap-4 w-full h-full max-w-5xl mx-auto">
      <div className="flex justify-between items-center bg-background p-2 border rounded-md shadow-sm">
        <div className="text-sm text-muted-foreground flex items-center gap-2">
          ...
          Type in Tanglish (e.g., 'vanakkam') and press space.
        </div>
      ...
      <div className="p-4 border rounded-md bg-muted/20 flex flex-col sm:flex-row items-center gap-4 justify-between mt-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
          <Button variant="outline" size="sm" onClick={handleConvertAndCopy} disabled={!text} className="whitespace-nowrap">
            <Copy className="w-4 h-4 mr-2" /> Copy
          </Button>
          <Button variant="outline" size="sm" onClick={downloadTxt} disabled={!text} className="whitespace-nowrap">
            <FileText className="w-4 h-4 mr-2" /> TXT
          </Button>
          <Button variant="outline" size="sm" onClick={downloadDocx} disabled={!text} className="whitespace-nowrap">
            <FileIcon className="w-4 h-4 mr-2 text-blue-600" /> DOCX
          </Button>
          <Button variant="outline" size="sm" onClick={downloadPdf} disabled={!text} className="whitespace-nowrap">
